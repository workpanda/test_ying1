...
function idf_unescape($v)
{
    ...
    $wd = substr($v, -1);
    return str_replace($wd . $wd, $wd, substr($v, 1, -1));
}
...
{
    static $Sg = [':' => ':1', ']' => ':2', '[' => ':3', '"' => ':4'];
    return strtr($v, $_a ? array_flip($Sg) : $Sg);
}
...
function checkbox($E, $Y, $Na, $sd = '', $te = '', $Ra = '', $td = '')
{
    $K =
        ...
        '>' .
        ($te ? script("qsl('input').onclick = function () { $te };", '') : '');
    return $sd != '' || $Ra
        ? '<label' . ($Ra ? " class='$Ra'" : '') . ">$K" . h($sd) . '</label>'
        : $K;
}
function optionlist($xe, $Nf = null, $oh = false)
{
    ...
    foreach ($xe as $od => $W) {
        ...
        foreach ($ye as $z => $X) {
            $K .=
                '<option' .
                ($oh || is_string($z) ? ' value="' . h($z) . '"' : '') .
                (($oh || is_string($z) ? (string) $z : $X) === $Nf
                    ? ' selected'
                    : '') .
                '>' .
                h($X);
...
function html_select($E, $xe, $Y = '', $se = true, $td = '')
{
    if ($se) {
        return "<select name='" .
            ...
            '</select>' .
            (is_string($se)
                ? script("qsl('select').onchange = function () { $se };", '')
                : '');
    }
    $K = '';
    foreach ($xe as $z => $X) {
...
function select_input($wa, $xe, $Y = '', $se = '', $Xe = '')
{
    ...
    return "<$_g$wa" .
        ($xe
            ? "><option value=''>$Xe" . optionlist($xe, $Y, true) . '</select>'
            : " size='10' value='" . h($Y) . "' placeholder='$Xe'>") .
        ($se ? script("qsl('$_g').onchange = $se;", '') : '');
}
function confirm($D = '', $Of = "qsl('input')")
{
    return script(
        "$Of.onclick = function () { return confirm('" .
            ($D ? js_escape($D) : lang(0)) .
            "'); };",
        ''
    );
}
function print_fieldset($u, $Ad, $xh = false)
{
    echo '<fieldset><legend>',
        "<a href='#fieldset-$u'>$Ad</a>",
        script("qsl('a').onclick = partial(toggle, 'fieldset-$u');", ''),
        '</legend>',
        "<div id='fieldset-$u'" . ($xh ? '' : " class='hidden'") . ">\n";
}
function bold($Ga, $Ra = '')
{
...
{
    $_SESSION['pwds'][$th][$O][$V] =
        $_COOKIE['adminer_key'] && is_string($G)
...
function get_password()
{
    $K = get_session('pwds');
    if (is_array($K)) {
        $K = $_COOKIE['adminer_key']
            ? decrypt_string($K[0], $_COOKIE['adminer_key'])
            : false;
    }
    return $K;
}
...
function where($Z, $o = [])
{
    ...
    foreach ((array) $Z['where'] as $z => $X) {
        ...
        $K[] =
            $d .
            ($y == 'sql' && is_numeric($X) && preg_match('~\.~', $X)
                ? ' LIKE ' . q($X)
                : ($y == 'mssql'
                    ? ' LIKE ' . q(preg_replace('~[_%[]~', '[\0]', $X))
                    : ' = ' . unconvert_field($o[$z], q($X))));
        if (
            $y == 'sql' &&
            preg_match('~char|text~', $o[$z]['type']) &&
            preg_match('~[^ -@]~', $X)
        ) {
...
{
    return "&where%5B$t%5D%5Bcol%5D=" .
        urlencode($d) .
        "&where%5B$t%5D%5Bop%5D=" .
        urlencode($Y !== null ? $ue : 'IS NULL') .
        "&where%5B$t%5D%5Bval%5D=" .
        urlencode($Y);
}
...
function cookie($E, $Y, $Dd = 2592000)
{
    ...
    return header(
        "Set-Cookie: $E=" .
            urlencode($Y) .
            ($Dd
                ? '; expires=' . gmdate('D, d M Y H:i:s', time() + $Dd) . ' GMT'
                : '') .
            '; path=' .
...
function queries_redirect($B, $D, $sf)
{
    ...
    return query_redirect($nf, $B, $D, $sf, false, !$sf, $Gg);
}
function format_time($hg)
{
    return lang(1, max(0, microtime(true) - $hg));
}
...
function get_file($z, $yb = false)
{
    ...
    foreach ($uc['error'] as $z => $m) {
        ...
        $E = $uc['name'][$z];
        $Og = $uc['tmp_name'][$z];
        $gb = file_get_contents(
            $yb && preg_match('~\.gz$~', $E) ? "compress.zlib://$Og" : $Og
        );
        if ($yb) {
            $hg = substr($gb, 0, 3);
            if (
                function_exists('iconv') &&
                preg_match("~^\xFE\xFF|^\xFF\xFE~", $hg, $yf)
            ) {
...
{
    return str_repeat("$Ue{0,65535}", $Bd / 65535) .
        "$Ue{0," .
        $Bd % 65535 .
        '}';
}
...
function shorten_utf8($lg, $Bd = 80, $pg = '')
{
    ...
    return h($C[1]) . $pg . (isset($C[2]) ? '' : '<i>…</i>');
}
function format_number($X)
{
    return strtr(
        number_format($X, 0, '.', lang(5)),
...
function hidden_fields($kf, $Yc = [], $df = '')
{
    $K = false;
    foreach ($kf as $z => $X) {
        if (!in_array($z, $Yc)) {
...
function enum_input($U, $wa, $n, $Y, $Yb = null)
{
    ...
    $K =
        $Yb !== null
            ? "<label><input type='$U'$wa value='$Yb'" .
                ((is_array($Y)
                        ? in_array($Yb, $Y)
                        : $Y === 0)
                    ? ' checked'
...
function input($n, $Y, $r)
{
    ...
    $Gc =
        (isset($_GET['select']) || $_f ? ['orig' => lang(8)] : []) +
        $c->editFunctions($n);
    $wa = " name='fields[$E]'";
    if ($n['type'] == 'enum') {
        echo h($Gc['']) . '<td>' . $c->editInput($_GET['edit'], $n, $wa, $Y);
    } else {
        $Pc = in_array($r, $Gc) || isset($Gc[$r]);
        echo (count($Gc) > 1
            ? "<select name='function[$E]'>" .
                optionlist($Gc, $r === null || $Pc ? $r : '') .
                '</select>' .
        ...
        if ($ed != '') {
            ...
            foreach ($Jd[1] as $t => $X) {
                ...
                echo " <label><input type='checkbox' name='fields[$E][$t]' value='" .
                    (1 << $t) .
                    "'" .
                    ($Na ? ' checked' : '') .
                    '>' .
            ...
            preg_match('~blob|bytea|raw|file~', $n['type']) &&
            ini_bool('file_uploads')
        ) {
            echo "<input type='file' name='fields-$E'>";
        } elseif (
            ($Eg = preg_match('~text|lob|memo~i', $n['type'])) ||
            preg_match("~\n~", $Y)
        ) {
            if ($Eg && $y != 'sqlite') {
                $wa .= " cols='50' rows='12'";
                ...
                $wa .=
                    " cols='30' rows='$M'" .
                    ($M == 1 ? " style='height: 1.2em;'" : '');
            }
            echo "<textarea$wa>" . h($Y) . '</textarea>';
        } elseif ($r == 'json' || preg_match('~^jsonb?$~', $n['type'])) {
            echo "<textarea$wa cols='50' rows='12' class='jush-js'>" .
                h($Y) .
                '</textarea>';
        } else {
            $Qd =
                !preg_match('~int~', $n['type']) &&
                preg_match('~^(\d+)(,(\d+))?$~', $n['length'], $C)
                    ? (preg_match('~binary~', $n['type']) ? 2 : 1) * $C[1] +
                        ($C[3] ? 1 : 0) +
                        ($C[2] && !$n['unsigned'] ? 1 : 0)
                    : ($ah[$n['type']]
                        ? $ah[$n['type']] + ($n['unsigned'] ? 0 : 1)
                        : 0);
            if (
                $y == 'sql' &&
                min_version(5.6) &&
                preg_match('~time~', $n['type'])
            ) {
                $Qd += 7;
            }
            echo '<input' .
                ((!$Pc || $r === '') &&
                preg_match('~(?<!o)int(?!er)~', $n['type']) &&
                !preg_match('~\[\]~', $n['full_type'])
                    ? " type='number'"
                    : '') .
                " value='" .
                h($Y) .
                "'" .
                ($Qd ? " data-maxlength='$Qd'" : '') .
                (preg_match('~char|binary~', $n['type']) && $Qd > 20
                    ? " size='40'"
                    : '') .
                "$wa>";
        }
...
function process_input($n)
{
    ...
    if ($r == 'orig') {
        return preg_match('~^CURRENT_TIMESTAMP~i', $n['on_update'])
            ? idf_escape($n['field'])
            : false;
    }
    if ($r == 'NULL') {
                ...
                'SELECT' .
                    limit(
                        ...
                        ' WHERE ' .
                            implode(
                                ' AND ',
                                $c->selectSearchProcess(fields($Q), [])
                            ),
                        1
                    )
            );
...
function select_value($X, $A, $n, $Fg)
{
    ...
    if (is_array($X)) {
        ...
        foreach ($X as $od => $W) {
            $K .=
                '<tr>' .
                ($X != array_values($X) ? '<th>' . h($od) : '') .
                '<td>' .
                select_value($W, $A, $n, $Fg);
        }
...
function is_mail($Vb)
{
    ...
    $Ue = "$va+(\\.$va+)*@($Jb?\\.)+$Jb";
    return is_string($Vb) && preg_match("(^$Ue(,\\s*$Ue)*\$)i", $Vb);
}
function is_url($lg)
{
    $Jb = '[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';
...
function count_rows($Q, $Z, $kd, $s)
{
    ...
    return $kd && ($y == 'sql' || count($s) == 1)
        ? 'SELECT COUNT(DISTINCT ' . implode(', ', $s) . ")$I"
        : 'SELECT COUNT(*)' .
                ($kd
                    ? " FROM (SELECT 1$I GROUP BY " . implode(', ', $s) . ') x'
                    : $I);
}
...
{
    return script(
        "mixin(qsl('select, input'), {onmouseover: function (event) { helpMouseover.call(this, event, $Za, $Xf) }, onmouseout: helpMouseout});",
        ''
    );
}
function edit_form($Q, $o, $L, $ih)
{
    ...
    page_header($ih ? lang(10) : lang(11), $m, ['select' => [$Q, $ug]], $ug);
...
';
    if (!$o) {
        ...
        foreach ($o as $E => $n) {
            ...
            $r = $_POST['save']
                ? (string) $_POST['function'][$E]
                : ($ih && preg_match('~^CURRENT_TIMESTAMP~i', $n['on_update'])
                    ? 'now'
                    : ($Y === false
                        ? null
                        : ($Y !== null
        ...
        if (!support('table')) {
            echo '<tr>' .
                ...
                html_select(
                    'field_funs[]',
                    $c->editFunctions(['null' => isset($_GET['select'])])
                ) .
    ...
    <g@��hy8��p.ef\n��eh��aX����mS��jBژQ\"�\r���K3�=>ǪAX�[,,\"'
                                                                                                                                                                                                                            <��?S�?�a=k�=��I���;�E8�P�p2�.23���'t;�����IC4'QyE*CC�Y�3�|SEd�lL0�k8Gp,)��(F�c+�S
                                                                                                                                                                                                                                /T)�<L��8��~b��HIH�~Cp`����q�J���\0thn7�2͊r��L�KlH+����>
                                                                                                                                                                                                                                n#EQF�E����\$^�(�F�{�}P����#�5�FL����>S�4O�R�F})�9R�S�Gk�Fh\n|\n�BKS�����=D�AE�1r�S��A1�V�mVUJ���;��HU~���X�PP5kD�*�ZhBd��>~+ha'{��2��c��?��=��D����E\\�!T\0�^�7V��=U��TuXS�T&{&��x��b��yՎ�5�Br�YUV_
                                                                                                                                                                                                                                `f�V6|V\\�\$_�\0g�H�E䪤T�s�E\n�]O�V�Qd�-�WU3T6YU�OFM,U{`�b�eϏU/
                                                                                                                                                                                                                                b�-dĮv�F=W�K6���vP��i��q��*tCR�T6��.�?�_�yT!J>����~61_1�Q�\$���+�\0G�>�[�\0[�Gt\"�)ooS{
                                                                                                                                                                                                                                ��oџp�-'pv�o�8p��m�.φpgK]N�[r�1�1ѽo�p\$�
                                                                                                                                                                                                                                <v�kS�r�>
                                                                                                                                                                                                                                    �q�SF1!B�K�v�^��u�UB�~@��
                                                                                                                                                                                                                                    �p��H�l;lQJ���ua6�7B-b���4�j���4Z�Q�Ā�\n���qIs`Tu9,(_��4P���9�
...
if (extension_loaded('pdo')) {
    class Min_PDO
    {
        ...
        function dsn($Ob, $V, $G, $xe = [])
        {
            $xe[PDO::ATTR_ERRMODE] = PDO::ERRMODE_SILENT;
            $xe[PDO::ATTR_STATEMENT_CLASS] = ['Min_PDOStatement'];
            try {
                $this->pdo = new PDO($Ob, $V, $G, $xe);
            } catch (Exception $hc) {
                auth_error(h($hc->getMessage()));
            }
            $this->server_info = @$this->pdo->getAttribute(
...
class Min_SQL
{
    ...
    function select($Q, $N, $Z, $s, $ze = [], $_ = 1, $F = 0, $gf = false)
    {
        global $c, $y;
        $kd = count($s) < count($N);
    ...
    {
        $I = 'FROM ' . table($Q);
        return queries('DELETE' . ($_ ? limit1($Q, $I, $of) : " $I$of"));
    }
    function update($Q, $P, $of, $_ = 0, $Rf = "\n")
    {
        ...
        return queries('UPDATE' . ($_ ? limit1($Q, $I, $of, $Rf) : " $I$of"));
    }
    function insert($Q, $P)
    {
        return queries(
...
class Adminer
{
    ...
    function selectQuery($I, $hg, $qc = false)
    {
        ...
        if (!$qc && ($zh = $l->warnings())) {
            ...
            $K =
                ...
                lang(35) .
                '</a>' .
                script("qsl('a').onclick = partial(toggle, '$u');", '') .
                "$K<div id='$u' class='hidden'>\n$zh</div>\n";
        }
        return "<p><code class='jush-$y'>" .
            h(str_replace("\n", ' ', $I)) .
    ...
    function selectVal($X, $A, $n, $Ge)
    {
        $K =
            $X === null
                ? '<i>NULL</i>'
                : (preg_match('~char|binary|boolean~', $n['type']) &&
                !preg_match('~var~', $n['type'])
                    ? "<code>$X</code>"
                    : $X);
    ...
    function tableStructurePrint($o)
    {
        echo "<div class='scrollable'>\n",
            ...
            '<thead><tr><th>' .
                ...
                '<td>' .
                lang(38) .
                (support('comment') ? '<td>' . lang(39) : '') .
                "</thead>\n";
        foreach ($o as $n) {
            echo '<tr' . odd() . '><th>' . h($n['field']),
                "<td><span title='" .
                    h($n['collation']) .
                    "'>" .
                    h($n['full_type']) .
                ...
                isset($n['default'])
                    ? " <span title='" .
                        ...
                        '</b>]</span>'
                    : '',
                support('comment') ? '<td>' . h($n['comment']) : '',
                "\n";
        }
        echo "</table>\n", "</div>\n";
    }
    ...
    function selectSearchPrint($Z, $e, $x)
    {
        ...
        foreach ($x as $t => $w) {
            if ($w['type'] == 'FULLTEXT') {
                echo '<div>(<i>' .
                implode('</i>, <i>', array_map('h', $w['columns'])) .
                '</i>) AGAINST',
                    " <input type='search' name='fulltext[$t]' value='" .
                        h($_GET['fulltext'][$t]) .
                        "'>",
        ...
        foreach (array_merge((array) $_GET['where'], [[]]) as $t => $X) {
            if (
                ...
                select_input(
                    ...
                    "<input type='search' name='where[$t][val]' value='" .
                        h($X['val']) .
                        "'>",
                    script(
    ...
    function selectOrderPrint($ze, $e, $x)
    {
        ...
        foreach ((array) $_GET['order'] as $z => $X) {
            if ($X != '') {
                echo '<div>' .
                select_input(" name='order[$t]'", $e, $X, 'selectFieldChange'),
                    checkbox(
                        "desc[$t]",
                        1,
                        isset($_GET['desc'][$z]),
        ...
        echo '<div>' .
        select_input(" name='order[$t]'", $e, '', 'selectAddRow'),
            checkbox("desc[$t]", 1, false, lang(48)) . "</div>\n",
            "</div></fieldset>\n";
    }
    ...
    function selectActionPrint($x)
    {
        echo '<fieldset><legend>' . lang(51) . '</legend><div>',
            "<input type='submit' value='" . lang(42) . "'>",
            " <span id='noindex' title='" . lang(52) . "'></span>",
            '<script' . nonce() . ">\n",
            'var indexColumns = ';
        $e = [];
        foreach ($x as $w) {
    ...
    function selectSearchProcess($o, $x)
    {
        ...
        foreach ($x as $t => $w) {
            if ($w['type'] == 'FULLTEXT' && $_GET['fulltext'][$t] != '') {
                $K[] =
                    'MATCH (' .
                    implode(', ', array_map('idf_escape', $w['columns'])) .
                    ') AGAINST (' .
                    q($_GET['fulltext'][$t]) .
        ...
        foreach ((array) $_GET['where'] as $z => $X) {
            if (
                ...
                if (preg_match('~IN$~', $X['op'])) {
                    ...
                    $db =
                        ' LIKE ' .
                        $this->processInput($o[$X['col']], "%$X[val]%");
                } elseif ($X['op'] == 'ILIKE %%') {
                    $db =
                        ' ILIKE ' .
                        $this->processInput($o[$X['col']], "%$X[val]%");
                ...
                if ($X['col'] != '') {
                    ...
                    foreach ($o as $E => $n) {
                        if (
                            ...
                            (!preg_match('~date|timestamp~', $n['type']) ||
                                preg_match('~^\d+-\d+-\d+~', $X['val']))
                        ) {
                            $Ya[] =
                                $df .
                                $l->convertSearch(idf_escape($E), $X, $n) .
                                $db;
                        }
                    }
    ...
    function messageQuery($I, $Gg, $qc = false)
    {
        ...
        if (strlen($I) > 1e6) {
            $I =
                preg_replace('~[\x80-\xFF]+$~', '', substr($I, 0, 1e6)) . "\n…";
        }
        $Tc[$_GET['db']][] = [$I, time(), $Gg];
        $fg = 'sql-' . count($Tc[$_GET['db']]);
        $K = "<a href='#$fg' class='toggle'>" . lang(53) . "</a>\n";
        if (!$qc && ($zh = $l->warnings())) {
            $u = 'warnings-' . count($Tc[$_GET['db']]);
            $K =
                "<a href='#$u' class='toggle'>" .
                lang(35) .
                "</a>, $K<div id='$u' class='hidden'>\n$zh</div>\n";
        }
        return " <span class='time'>" .
            @date('H:i:s') .
            '</span>' .
            " $K<div id='$fg' class='hidden'><pre><code class='jush-$y'>" .
            shorten_utf8($I, 1000) .
            '</code></pre>' .
            ($Gg ? " <span class='time'>($Gg)</span>" : '') .
            (support('sql')
                ? '<p><a href="' .
                    h(
                        str_replace(
    ...
    function editFunctions($n)
    {
        ...
        foreach ($Rb as $z => $Gc) {
            ...
            if (
                $z &&
                !preg_match('~set|blob|bytea|raw|file|bool~', $n['type'])
            ) {
                $K .= '/SQL';
            }
        }
    ...
    function editInput($Q, $n, $wa, $Y)
    {
        if ($n['type'] == 'enum') {
            return (isset($_GET['select'])
                ? "<label><input type='radio'$wa value='-1' checked><i>" .
                    lang(8) .
                    '</i></label> '
                : '') .
                ($n['null']
                    ? "<label><input type='radio'$wa value=''" .
                        ($Y !== null || isset($_GET['select'])
                            ? ''
                            : ' checked') .
                        '><i>NULL</i></label> '
                    : '') .
                enum_input('radio', $wa, $n, $Y, 0);
        }
        return '';
    }
                    ...
                    if ($_POST['format'] != 'sql') {
                        ...
                        if (!$fd) {
                            $fd =
                                ...
                                table($Q) .
                                ' (' .
                                implode(', ', array_map('idf_escape', $pd)) .
                                ') VALUES';
                        }
                        foreach ($L as $z => $X) {
                            $n = $o[$z];
                        ...
                        if (!$Ia) {
                            ...
                            strlen($Ia) + 4 + strlen($If) + strlen($pg) <
                            $Ld
                        ) {
                            $Ia .= ",$If";
                        } else {
    ...
    {
        return friendly_url(
            $Xc != '' ? $Xc : (SERVER != '' ? SERVER : 'localhost')
        );
    }
    function dumpHeaders($Xc, $Xd = false)
    {
        $Ie = $_POST['output'];
        $nc = preg_match('~sql~', $_POST['format'])
            ? 'sql'
            : ($Xd
                ? 'tar'
                : 'csv');
    ...
    function homepage()
    {
        ...
        ($_GET['ns'] == '' && support('database')
            ? '<a href="' . h(ME) . 'database=">' . lang(56) . "</a>\n"
            : ''),
            support('scheme')
                ? "<a href='" .
                    h(ME) .
                    "scheme='>" .
                    ($_GET['ns'] != '' ? lang(57) : lang(58)) .
                    "</a>\n"
                : '',
            $_GET['ns'] !== ''
                ? '<a href="' . h(ME) . 'schema=">' . lang(59) . "</a>\n"
                : '',
            support('privileges')
                ? "<a href='" . h(ME) . "privileges='>" . lang(60) . "</a>\n"
                : '';
        return true;
    }
...
<a href="https://www.adminer.org/#download"',
            target_blank(),
            ' id="version">',
            version_compare($fa, $_COOKIE['adminer_version']) < 0
...
';
        if ($Wd == 'auth') {
            ...
            echo script_src(
                preg_replace('~\\?.*~', '', ME) . '?file=jush.js&version=4.8.1'
            );
            if (support('sql')) {
                echo '<script',
                    nonce(),
...
';
                ...
                echo 'bodyLoad(\'',
                    is_object($g)
                        ? preg_replace('~^(\d\.?\d).*~s', '\1', $Sf)
                        : '',
                    '\'',
                    preg_match('~MariaDB~', $Sf) ? ', true' : '',
                    ');
</script>
';
            }
...
$c = function_exists('adminer_object') ? adminer_object() : new Adminer();
$Kb = ['server' => 'MySQL'] + $Kb;
if (!defined('DRIVER')) {
    ...
    if (extension_loaded('mysqli')) {
        class Min_DB extends MySQLi
        {
            ...
            function connect(
                ...
                $K = @$this->real_connect(
                    $O != '' ? $Vc : ini_get('mysqli.default_host'),
                    $O . $V != '' ? $V : ini_get('mysqli.default_user'),
                    $O . $V . $G != '' ? $G : ini_get('mysqli.default_pw'),
                    $ub,
                    is_numeric($Ye) ? $Ye : ini_get('mysqli.default_port'),
                    !is_numeric($Ye) ? $Ye : $ag,
                    $gg ? 64 : 0
                );
        ...
        class Min_DB
        {
            ...
            {
                $J = @$bh
                    ? mysql_unbuffered_query($I, $this->_link)
                    : mysql_query($I, $this->_link);
                $this->error = '';
                if (!$J) {
        ...
        class Min_DB extends Min_PDO
        {
            ...
            {
                global $c;
                $xe = [PDO::MYSQL_ATTR_LOCAL_INFILE => false];
                $gg = $c->connectSsl();
                if ($gg) {
                    if (!empty($gg['key'])) {
    ...
    class Min_Driver extends Min_SQL
    {
        ...
        function convertSearch($v, $X, $n)
        {
            return preg_match('~char|text|enum|set~', $n['type']) &&
                !preg_match('~^utf8~', $n['collation']) &&
                preg_match('~[\x80-\xFF]~', $X['val'])
                ? "CONVERT($v USING " . charset($this->_conn) . ')'
                : $v;
        }
        function warnings()
        {
        ...
        function tableHelp($E)
        {
            ...
            if (information_schema(DB)) {
                return strtolower(
                    $Hd
                        ? "information-schema-$E-table/"
                        : str_replace('_', '-', $E) . '-table.html'
                );
            }
            if (DB == 'mysql') {
    ...
    function connect()
    {
        ...
        if (
            function_exists('iconv') &&
            !is_utf8($K) &&
            strlen($If = iconv('windows-1250', 'utf-8', $K)) > strlen($K)
        ) {
            $K = $If;
        }
        return $K;
    ...
    {
        return " $I$Z" .
            ($_ !== null ? $Rf . "LIMIT $_" . ($ke ? " OFFSET $ke" : '') : '');
    }
    function limit1($Q, $I, $Z, $Rf = "\n")
    {
        return limit($I, $Z, 1, 0, $Rf);
    }
                ...
                $rc && min_version(5)
                    ? 'SELECT TABLE_NAME AS Name, ENGINE AS Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() ' .
                        ($E != ''
                            ...
                            : 'ORDER BY Name')
                    : 'SHOW TABLE STATUS' .
                        ($E != '' ? ' LIKE ' . q(addcslashes($E, '%_\\')) : '')
            )
            as $L
        ) {
            if ($L['Engine'] == 'InnoDB') {
                $L['Comment'] = preg_replace(
                    '~(?:(.+); )?InnoDB free: .*~',
                    '\1',
                    $L['Comment']
                );
    ...
    function fields($Q)
    {
        ...
        foreach (get_rows('SHOW FULL COLUMNS FROM ' . table($Q)) as $L) {
            ...
            $K[$L['Field']] = [
                ...
                'default' =>
                    $L['Default'] != '' || preg_match('~char|set~', $C[1])
                        ? (preg_match('~text~', $C[1])
                            ? stripslashes(
                                preg_replace("~^'(.*)'\$~", '\1', $L['Default'])
                            )
                            : $L['Default'])
                        : null,
                'null' => $L['Null'] == 'YES',
                'auto_increment' => $L['Extra'] == 'auto_increment',
                'on_update' => preg_match('~^on update (.+)~i', $L['Extra'], $C)
                    ? $C[1]
                    : '',
                'collation' => $L['Collation'],
                'privileges' => array_flip(
                    preg_split('~, *~', $L['Privileges'])
                ),
                'comment' => $L['Comment'],
                'primary' => $L['Key'] == 'PRI',
    ...
    function indexes($Q, $h = null)
    {
        ...
        foreach (get_rows('SHOW INDEX FROM ' . table($Q), $h) as $L) {
            ...
            $K[$E]['type'] =
                $E == 'PRIMARY'
                    ? 'PRIMARY'
                    : ($L['Index_type'] == 'FULLTEXT'
                        ? 'FULLTEXT'
                        : ($L['Non_unique']
                            ? ($L['Index_type'] == 'SPATIAL'
                                ? 'SPATIAL'
                                : 'INDEX')
                            : 'UNIQUE'));
            $K[$E]['columns'][] = $L['Column_name'];
            $K[$E]['lengths'][] =
                $L['Index_type'] == 'SPATIAL' ? null : $L['Sub_part'];
            $K[$E]['descs'][] = null;
        }
        return $K;
    }
    function foreign_keys($Q)
    {
        ...
        if ($lb) {
            preg_match_all(
                "~CONSTRAINT ($Ue) FOREIGN KEY ?\\(((?:$Ue,? ?)+)\\) REFERENCES ($Ue)(?:\\.($Ue))? \\(((?:$Ue,? ?)+)\\)(?: ON DELETE ($re))?(?: ON UPDATE ($re))?~",
                $lb,
                $Jd,
                PREG_SET_ORDER
            );
            foreach ($Jd as $C) {
                ...
                $K[idf_unescape($C[1])] = [
                    ...
                    'source' => array_map('idf_unescape', $bg[0]),
                    'target' => array_map('idf_unescape', $Ag[0]),
    ...
    function view($E)
    {
        ...
        return [
            'select' => preg_replace(
                '~^(?:[^`]|`[^`]*`)*\s+AS\s+~isU',
                '',
                $g->result('SHOW CREATE VIEW ' . table($E), 1)
            ),
        ];
    }
    function collations()
    ...
    {
        return (min_version(5) && $k == 'information_schema') ||
            (min_version(5.5) && $k == 'performance_schema');
    }
    function error()
    {
        global $g;
    ...
    {
        return queries('DROP VIEW ' . implode(', ', array_map('table', $wh)));
    }
        ...
        if (!$zf || queries('RENAME TABLE ' . implode(', ', $zf))) {
            ...
            foreach ($Bb as $E => $vh) {
                if (
                    !queries(
                        "CREATE VIEW $E AS " .
                            str_replace(" $k.", ' ', $vh['select'])
                    ) ||
                    !queries("DROP VIEW $k.$E")
                ) {
                    return false;
                }
            }
    ...
    function copy_tables($S, $wh, $Ag)
    {
        ...
        foreach ($S as $Q) {
            $E =
                $Ag == DB
                    ? table("copy_$Q")
                    : idf_escape($Ag) . '.' . table($Q);
            if (
                ($_POST['overwrite'] &&
                    !queries("\nDROP TABLE IF EXISTS $E")) ||
                !queries("CREATE TABLE $E LIKE " . table($Q)) ||
        ...
        foreach ($wh as $Q) {
            ...
            $vh = view($Q);
            if (
                ($_POST['overwrite'] && !queries("DROP VIEW IF EXISTS $E")) ||
                !queries("CREATE VIEW $E AS $vh[select]")
            ) {
                return false;
            }
        }
    ...
    function routine($E, $U)
    {
        ...
        $Zg =
            '((' .
            implode('|', array_merge(array_keys($ah), $qa)) .
        ...
        foreach ($Jd as $Le) {
            $o[] = [
                'field' => str_replace('``', '`', $Le[2]) . $Le[3],
                'type' => strtolower($Le[5]),
                ...
                'unsigned' => strtolower(
                    preg_replace('~\s+~', ' ', trim("$Le[8] $Le[7]"))
                ),
                'null' => 1,
                'full_type' => $Le[4],
                'inout' => strtoupper($Le[1]),
                'collation' => strtolower($Le[9]),
            ];
        }
        if ($U != 'FUNCTION') {
            return ['fields' => $o, 'definition' => $C[11]];
        }
        return [
            'fields' => $o,
            'returns' => [
    ...
    function explain($g, $I)
    {
        return $g->query(
            'EXPLAIN ' .
                (min_version(5.1) && !min_version(5.7) ? 'PARTITIONS ' : '') .
                $I
        );
    }
    ...
    function driver_config()
    {
        ...
        return [
            ...
            'types' => $ah,
            'structured_types' => $mg,
            'unsigned' => ['unsigned', 'zerofill', 'unsigned zerofill'],
            'operators' => [
                '=',
                '<',
                '>',
...
$fa = '4.8.1';
function page_header($Jg, $m = '', $Ha = [], $Kg = '')
{
    global $a, $fa, $c, $Kb, $y;
    page_headers();
    if (is_ajax() && $m) {
...
<link rel="stylesheet" type="text/css" href="',
        h(preg_replace('~\\?.*~', '', ME) . '?file=default.css&version=4.8.1'),
        '">
',
        script_src(
            preg_replace('~\\?.*~', '', ME) . '?file=functions.js&version=4.8.1'
        );
    if ($c->head()) {
        echo '<link rel="shortcut icon" type="image/x-icon" href="',
            h(
                preg_replace('~\\?.*~', '', ME) .
                    '?file=favicon.ico&version=4.8.1'
            ),
            '">
...
mixin(document.body, {onkeydown: bodyKeydown, onclick: bodyClick',
        isset($_COOKIE['adminer_version'])
            ? ''
            : ", onload: partial(verifyVersion, '$fa', '" .
                js_escape(ME) .
                "', '" .
                get_token() .
                "')";
...
if ($Ha !== null) {
    ...
    if ($Ha === false) {
        ...
        echo "<a href='" .
            h($A) .
            "' accesskey='1' title='Alt+Shift+1'>$O</a> &raquo; ";
...
{
    $W = array_values(
        unpack('V*', str_pad($If, 4 * ceil(strlen($If) / 4), "\0"))
    );
    if ($yh) {
        $W[] = strlen($If);
    }
...
if (
    !is_object($g) ||
    ($Fd = $c->login($_GET['username'], get_password())) !== true
) {
    $m = is_string($g) ? h($g) : (is_string($Fd) ? $Fd : lang(87));
    auth_error(
        $m . (preg_match('~^ | $~', get_password()) ? '<br>' . lang(88) : '')
    );
}
...
function select($J, $h = null, $Be = [], $_ = 0)
{
    global $y;
    $Ed = [];
    $x = [];
            ...
            for ($nd = 0; $nd < count($L); $nd++) {
                ...
                echo '<th' .
                    ...
                    ($Be
                        ? doc_link([
                            'sql' =>
                                'explain-output.html#explain_' . strtolower($E),
                            'mariadb' =>
...
function set_adminer_settings($Wf)
{
    return cookie(
        'adminer_settings',
        http_build_query($Wf + adminer_settings())
    );
}
...
function edit_type($z, $n, $Xa, $Bc = [], $pc = [])
{
    global $mg, $ah, $hh, $re;
    $U = $n['type'];
    echo '<td><select name="',
    ...
    echo optionlist(array_merge($pc, $mg), $U),
        '</select><td><input name="',
        h($z),
        '[length]" value="',
        h($n['length']),
        '" size="3"',
        !$n['length'] && preg_match('~var(char|binary)$~', $U)
            ? " class='required'"
            : '';
    echo ' aria-labelledby="label-length"><td class="options">',
        "<select name='" .
            h($z) .
            "[collation]'" .
            (preg_match('~(char|text|enum|set)$~', $U)
                ? ''
                : " class='hidden'") .
            '><option value="">(' .
            lang(95) .
            ')' .
            optionlist($Xa, $n['collation']) .
            '</select>',
        $hh
            ? "<select name='" .
                h($z) .
                "[unsigned]'" .
                (!$U || preg_match(number_type(), $U)
                    ? ''
                    : " class='hidden'") .
                '><option>' .
                optionlist($hh, $n['unsigned']) .
                '</select>'
            : '',
        isset($n['on_update'])
            ? "<select name='" .
                h($z) .
                "[on_update]'" .
                (preg_match('~timestamp|datetime~', $U)
                    ? ''
                    : " class='hidden'") .
                '>' .
                optionlist(
                    ['' => '(' . lang(96) . ')', 'CURRENT_TIMESTAMP'],
                    preg_match('~^CURRENT_TIMESTAMP~i', $n['on_update'])
                        ? 'CURRENT_TIMESTAMP'
                        : $n['on_update']
                ) .
                '</select>'
...
function process_type($n, $Va = 'COLLATE')
{
    global $hh;
    return " $n[type]" .
        process_length($n['length']) .
        (preg_match(number_type(), $n['type']) && in_array($n['unsigned'], $hh)
            ? " $n[unsigned]"
            : '') .
        (preg_match('~char|text|enum|set~', $n['type']) && $n['collation']
            ? " $Va " . q($n['collation'])
            : '');
}
function process_field($n, $Yg)
{
    return [
        ...
        default_value($n),
        preg_match('~timestamp|datetime~', $n['type']) && $n['on_update']
            ? " ON UPDATE $n[on_update]"
            : '',
        support('comment') && $n['comment'] != ''
            ? ' COMMENT ' . q($n['comment'])
            : '',
...
function default_value($n)
{
    ...
    return $zb === null
        ? ''
        : ' DEFAULT ' .
                (preg_match('~char|binary|text|enum|set~', $n['type']) ||
                preg_match('~^(?![a-z])~i', $zb)
                    ? q($zb)
                    : $zb);
}
...
function edit_fields($o, $Xa, $U = 'TABLE', $Bc = [])
{
    global $dd;
    $o = array_values($o);
    $_b = ($_POST
            ? $_POST['defaults']
            : adminer_setting('defaults'))
        ? ''
        : " class='hidden'";
    $cb = ($_POST
            ? $_POST['comments']
...
';
    ...
    echo '<th id="label-name">',
        $U == 'TABLE' ? lang(98) : lang(99),
        '<td id="label-type">',
        lang(38),
        '<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;"></textarea>',
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="',
            lang(40),
            '">AI</acronym>',
            doc_link([
            ...
            '>',
            lang(41),
            support('comment') ? "<td id='label-comment'$cb>" . lang(39) : '';
    }
    echo '<td>',
        "<input type='image' class='icon' name='add[" .
            (support('move_col') ? 0 : count($o)) .
            "]' src='" .
            h(
...
',
    ...
    foreach ($o as $t => $n) {
        ...
        $Hb =
            (isset($_POST['add'][$t - 1]) ||
                (isset($n['field']) && !$_POST['drop_col'][$t])) &&
            (support('drop_col') || $Ce == '');
        echo '<tr',
...
',
        ...
        if ($U == 'TABLE') {
            ...
            echo ' aria-labelledby="label-ai"></label><td',
                ...
                support('comment')
                    ? "<td$cb><input name='fields[$t][comment]' value='" .
                        h($n['comment']) .
                        "' data-maxlength='" .
        ...
        echo '<td>',
            support('move_col')
                ? "<input type='image' class='icon' name='add[$t]' src='" .
                    h(
                        preg_replace('~\\?.*~', '', ME) .
                            '?file=plus.gif&version=4.8.1'
                    ) .
                    "' alt='+' title='" .
                    lang(102) .
                    "'> " .
                    "<input type='image' class='icon' name='up[$t]' src='" .
                    h(
                        preg_replace('~\\?.*~', '', ME) .
                            '?file=up.gif&version=4.8.1'
                    ) .
                    "' alt='↑' title='" .
                    lang(103) .
                    "'> " .
                    "<input type='image' class='icon' name='down[$t]' src='" .
                    h(
                        preg_replace('~\\?.*~', '', ME) .
                            '?file=down.gif&version=4.8.1'
                    ) .
                    "' alt='↓' title='" .
                    lang(104) .
                    "'> "
                : '',
            $Ce == '' || support('drop_col')
                ? "<input type='image' class='icon' name='drop_col[$t]' src='" .
                    h(
                        preg_replace('~\\?.*~', '', ME) .
                            '?file=cross.gif&version=4.8.1'
                    ) .
                    "' alt='x' title='" .
                    lang(105) .
                    "'>"
...
function grant($Hc, $jf, $e, $qe)
{
    ...
    if ($jf == ['ALL PRIVILEGES', 'GRANT OPTION']) {
        return $Hc == 'GRANT'
            ? queries("$Hc ALL PRIVILEGES$qe WITH GRANT OPTION")
            : queries("$Hc ALL PRIVILEGES$qe") &&
                    queries("$Hc GRANT OPTION$qe");
    }
    return queries(
        "$Hc " .
            preg_replace(
...
function drop_create($Lb, $i, $Mb, $Dg, $Nb, $B, $Td, $Rd, $Sd, $ne, $ce)
{
    if ($_POST['drop']) {
        ...
        queries_redirect(
            $B,
            $Rd,
            queries($Dg) && queries($Nb) && queries($Lb) && queries($i)
        );
    }
}
...
function create_routine($Ff, $L)
{
    ...
    foreach ($o as $n) {
        if ($n['field'] != '') {
            $P[] =
                (preg_match("~^($dd)\$~", $n['inout']) ? "$n[inout] " : '') .
                idf_escape($n['field']) .
                process_type($n, 'CHARACTER SET');
        }
    }
    $Ab = rtrim("\n$L[definition]", ';');
    return "CREATE $Ff " .
        idf_escape(trim($L['name'])) .
        ' (' .
        implode(', ', $P) .
        ')' .
        (isset($_GET['function'])
            ? ' RETURNS' . process_type($L['returns'], 'CHARACTER SET')
            : '') .
...
function format_foreign_key($p)
{
    ...
    $ge = $p['ns'];
    return ' FOREIGN KEY (' .
        implode(', ', array_map('idf_escape', $p['source'])) .
        ') REFERENCES ' .
        ($k != '' && $k != $_GET['db'] ? idf_escape($k) . '.' : '') .
        ($ge != '' && $ge != $_GET['ns'] ? idf_escape($ge) . '.' : '') .
        table($p['table']) .
        ' (' .
        implode(', ', array_map('idf_escape', $p['target'])) .
        ')' .
        (preg_match("~^($re)\$~", $p['on_delete'])
            ? " ON DELETE $p[on_delete]"
            : '') .
        (preg_match("~^($re)\$~", $p['on_update'])
            ? " ON UPDATE $p[on_update]"
            : '');
}
function tar_file($vc, $Ng)
{
    ...
    $Ng->send();
    echo str_repeat("\0", 511 - (($Ng->size + 511) % 512));
}
...
function doc_link($Te, $Eg = '<sup>?</sup>')
{
    ...
    $lh = [
        'sql' => "https://dev.mysql.com/doc/refman/$uh/en/",
        'sqlite' => 'https://www.sqlite.org/',
        'pgsql' => "https://www.postgresql.org/docs/$uh/",
        'mssql' => 'https://msdn.microsoft.com/library/',
        'oracle' =>
            'https://www.oracle.com/pls/topic/lookup?ctx=db' .
            preg_replace('~^.* (\d+)\.(\d+)\.\d+\.\d+\.\d+.*~s', '\1\2', $Sf) .
            '&id=',
    ];
    if (preg_match('~MariaDB~', $Sf)) {
        $lh['sql'] = 'https://mariadb.com/kb/en/library/';
        $Te['sql'] = isset($Te['mariadb'])
            ? $Te['mariadb']
            : str_replace('.html', '/', $Te['sql']);
    }
    return $Te[$y]
        ? "<a href='" . h($lh[$y] . $Te[$y]) . "'" . target_blank() . ">$Eg</a>"
        : '';
}
            ...
            foreach ($j as $k => $S) {
                ...
                (support('database')
                    ? "<a href='$Ef" .
                        ...
                        "'>$Wa</a>"
                    : $Wa),
                    "<td align='right'><a href='$Ef&amp;schema=' id='tables-" .
                        h($k) .
                        "' title='" .
                        lang(59) .
                        "'>" .
...
if (isset($_GET['download'])) {
    ...
    header(
        'Content-Disposition: attachment; filename=' .
            friendly_url("$b-" . implode('_', $_GET['where'])) .
            '.' .
        ...
        if (fk_support($R)) {
            ...
            if ($Bc) {
                ...
                foreach ($Bc as $E => $p) {
                    echo "<tr title='" . h($E) . "'>",
                        '<th><i>' .
                            implode('</i>, <i>', array_map('h', $p['source'])) .
                            '</i>',
                        "<td><a href='" .
                            h(
                                $p['db'] != ''
                            ...
                            '</a>',
                        '(<i>' .
                            implode('</i>, <i>', array_map('h', $p['target'])) .
                            '</i>)',
                        '<td>' . h($p['on_delete']) . "\n",
                        '<td>' . h($p['on_update']) . "\n",
                        '<td><a href="' .
    ...
    page_header(lang(59), '', [], h(DB . ($_GET['ns'] ? ".$_GET[ns]" : '')));
    $vg = [];
    $wg = [];
    $da = $_GET['schema']
        ? $_GET['schema']
    ...
    foreach (table_status('', true) as $Q => $R) {
        ...
        foreach ($c->foreignKeys($Q) as $X) {
            if (!$X['db']) {
                ...
                $Kf[$Q]['references'][$X['table']][(string) $yd] = [
                    $X['source'],
                    $X['target'],
                ];
            ...
            foreach ($xf as $yd => $e) {
                ...
                foreach ($e as $Ag) {
                    echo "\n<div class='references' title='" .
                        ...
                        h(
                            preg_replace('~\\?.*~', '', ME) .
                                '?file=arrow.gif) no-repeat right center;&version=4.8.1'
                        ) .
        ...
        foreach ((array) $Q['references'] as $Bg => $xf) {
            foreach ($xf as $yd => $tf) {
                ...
                foreach ($tf[0] as $z => $bg) {
                    ...
                    $bf =
                        $Kf[$Bg]['pos'][0] +
                        $Kf[$Bg]['fields'][$tf[1][$z]]['pos'];
                    $Vd = min($Vd, $af, $bf);
                    $Nd = max($Nd, $af, $bf);
                }
                echo "<div class='references' id='refl$yd' style='left: $yd" .
                    "em; top: $Vd" .
...
";
        ...
        foreach ((array) $j as $k) {
            ...
            if ($g->select_db($k)) {
                if (
                    $ld &&
                    preg_match('~CREATE~', $ng) &&
                    ($i = $g->result(
                        'SHOW CREATE DATABASE ' . idf_escape($k),
                        1
                    ))
                ) {
                    set_utf8mb4($i);
                ...
                if ($ld) {
                    ...
                    $He = '';
                    if ($_POST['routines']) {
                        foreach (['FUNCTION', 'PROCEDURE'] as $Ff) {
                            foreach (
                                get_rows(
                                    "SHOW $Ff STATUS WHERE Db = " . q($k),
                                    null,
                                    '-- '
                                )
                                as $L
...
';
    ...
    echo '<tr><th>' .
        lang(134) .
        '<td>' .
        html_select('output', $c->dumpOutput(), $L['output'], 0) .
        "\n";
    echo '<tr><th>' .
        lang(135) .
        '<td>' .
        html_select('format', $c->dumpFormat(), $L['format'], 0) .
        "\n";
    echo $y == 'sqlite'
    ? ''
    : '<tr><th>' .
        lang(26) .
        '<td>' .
        html_select('db_style', $wb, $L['db_style']) .
        (support('routine')
            ? checkbox('routines', 1, $L['routines'], lang(136))
            : '') .
        (support('event')
            ? checkbox('events', 1, $L['events'], lang(137))
            : ''),
        '<tr><th>' .
            lang(117) .
            '<td>' .
            html_select('table_style', $xg, $L['table_style']) .
            checkbox('auto_increment', 1, $L['auto_increment'], lang(40)) .
            (support('trigger')
                ? checkbox('triggers', 1, $L['triggers'], lang(131))
                : ''),
        '<tr><th>' .
            lang(138) .
            '<td>' .
            html_select('data_style', $tb, $L['data_style']),
        '</table>
...
',
    ...
    if (DB != '') {
        $Na = $b != '' ? '' : ' checked';
        echo '<thead><tr>',
            "<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$Na>" .
                lang(117) .
                '</label>' .
                script(
            ...
            "<th style='text-align: right;'><label class='block'>" .
                lang(138) .
                "<input type='checkbox' id='check-data'$Na></label>" .
                script(
        ...
        foreach ($yg as $E => $U) {
            ...
            if ($U !== null && !preg_match('~table~i', $U)) {
                ...
                echo "$gf<td align='right'><label class='block'><span id='Rows-" .
                    h($E) .
                    "'></span>" .
                    checkbox('data[]', $E, $Na) .
                    "</label>\n";
...
';
    ...
    if (!$m && $_POST) {
        ...
        if (!isset($_GET['import'])) {
            ...
            $q = @fopen(
                file_exists($eg) ? $eg : "compress.zlib://$eg.gz",
                'rb'
            );
        ...
        if (is_string($I)) {
            if (function_exists('memory_get_usage')) {
                @ini_set(
                    ...
                    max(
                        ...
                        2 * strlen($I) + memory_get_usage() + 8e6
                    )
                );
            }
            if ($I != '' && strlen($I) < 1e6) {
            ...
            while ($I != '') {
                if (
                    !$ke &&
                    preg_match("~^$cg*+DELIMITER\\s+(\\S+)~i", $I, $C)
                ) {
                    $Cb = $C[1];
                    ...
                    if (!$Dc && $q && !feof($q)) {
                        ...
                        if ($Dc && rtrim($Dc) != $Cb) {
                            ...
                            $ab++;
                            $gf =
                                "<pre id='sql-$ab'><code class='jush-$y'>" .
                                $c->sqlCommandQuery($H) .
                                "</code></pre>\n";
                            if (
                                $y == 'sqlite' &&
                                preg_match("~^$cg*+ATTACH\\b~i", $H, $C)
                            ) {
                                echo $gf,
                                    "<p class='error'>" . lang(140) . "\n";
                                    ...
                                    if ($g->error) {
                                        ...
                                        if ($zh) {
                                            $Gg .=
                                                ...
                                                '</a>' .
                                                script(
                                                    "qsl('a').onclick = partial(toggle, '$_h');",
                                                    ''
                                                );
                                        }
                                        $lc = null;
                                        ...
                                        if (is_object($J)) {
                                            ...
                                            if (!$_POST['only_errors']) {
                                                ...
                                                echo ", <a href='#$u'>" .
                                                    ...
                                                    lang(62) .
                                                    "'><input type='hidden' name='token' value='$T'></span>\n" .
                                                    "</form>\n";
                                            }
                                        } else {
                                            if (
                                                preg_match(
                                                    "~^$cg*+(CREATE|DROP|ALTER)$cg++(DATABASE|SCHEMA)\\b~i",
                                                    $H
                                                )
                                            ) {
                                                restart_session();
            ...
            if ($Yb) {
                ...
                echo "<p class='message'>" . lang(146, $ab - count($dc)),
                    " <span class='time'>(" . format_time($Qg) . ")</span>\n";
            } elseif ($dc && $ab > 1) {
                echo "<p class='error'>" .
                    lang(141) .
                    ': ' .
...
';
    ...
    $_POST
        ? $_POST['error_stops']
        : isset($_GET['import']) || $_GET['error_stops'],
    lang(154)
) . "\n",
        checkbox(
            'only_errors',
            ...
            $_POST
                ? $_POST['only_errors']
                : isset($_GET['import']) || $_GET['only_errors'],
            lang(155)
        ) . "\n",
        "<input type='hidden' name='token' value='$T'>\n";
...
';
    ...
    $Z = isset($_GET['select'])
        ? ($_POST['check'] && count($_POST['check']) == 1
            ? where_check($_POST['check'][0], $o)
            : '')
        : where($_GET, $o);
    $ih = isset($_GET['select']) ? $_POST['edit'] : $Z;
    foreach ($o as $E => $n) {
        if (
            !isset($n['privileges'][$ih ? 'update' : 'insert']) ||
            $c->fieldName($n) == '' ||
            $n['generated']
        ) {
            unset($o[$E]);
    ...
    if ($_POST && !process_fields($L['fields']) && !$m) {
        if ($_POST['drop']) {
            ...
            if ($Oe[$L['partition_by']]) {
                ...
                $Qe .=
                    "\nPARTITION BY $L[partition_by]($L[partition])" .
                    ($Re
                        ? ' (' . implode(',', $Re) . "\n)"
                        : ($L['partitions']
            ...
            queries_redirect(
                ...
                alter_table(
                    ...
                    $L['Engine'] && $L['Engine'] != $R['Engine']
                        ? $L['Engine']
                        : '',
                    $L['Collation'] && $L['Collation'] != $R['Collation']
                        ? $L['Collation']
                        : '',
                    $L['Auto_increment'] != ''
                        ? number($L['Auto_increment'])
                        : '',
                    $Qe
                )
            );
    ...
    page_header($b != '' ? lang(33) : lang(63), $m, ['table' => $b], h($b));
    if (!$_POST) {
        $L = [
        ...
        if ($b != '') {
            ...
            if (support('partitioning')) {
                $Fc =
                    ...
                    q($b);
                $J = $g->query(
                    "SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $Fc ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1"
                );
                list(
                    $L['partition_by'],
                    $L['partitions'],
                ...
                $Re = get_key_vals(
                    "SELECT PARTITION_NAME, PARTITION_DESCRIPTION $Fc AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION"
                );
                $Re[''] = '';
...
';
    if (support('columns') || $b == '') {
        echo lang(165),
            ': <input name="name" data-maxlength="64" value="',
            h($L['name']),
            '" autocapitalize="off">
';
        if ($b == '' && !$_POST) {
...
',
            lang(40),
            ': <input type="number" name="Auto_increment" size="6" value="',
            h($L['Auto_increment']),
            '">
',
            checkbox(
...
',
            lang(170),
            ': <input type="number" name="partitions" class="size',
            $Pe || !$L['partition_by'] ? ' hidden' : '',
            '" value="',
            h($L['partitions']),
            '">
            ...
            if (in_array($w['type'], $bd)) {
                ...
                if ($e) {
                    ...
                    if ($kc) {
                        ...
                        if (
                            $w['type'] == $kc['type'] &&
                            array_values($kc['columns']) === $e &&
                            (!$kc['lengths'] ||
                                array_values($kc['lengths']) === $Cd) &&
                            array_values($kc['descs']) === $Eb
                        ) {
                            unset($x[$E]);
                            continue;
                        }
...
<th id="label-type">',
        lang(174),
        '<th><input type="submit" class="wayoff">',
        lang(175),
        '<th id="label-name">',
        lang(176),
        '<th><noscript>',
...
';
    if ($ff) {
        ...
        foreach ($ff['columns'] as $z => $d) {
            echo select_input(' disabled', $o, $d),
    ...
    foreach ($L['indexes'] as $w) {
        if (!$_POST['drop_col'] || $nd != key($_POST['drop_col'])) {
            ...
            html_select(
                ...
                [-1 => ''] + $bd,
                $w['type'],
                $nd == count($L['indexes']) ? 'indexesAddRow.call(this);' : 1,
                'label-type'
            ),
                '<td>';
            ksort($w['columns']);
            ...
            foreach ($w['columns'] as $z => $d) {
                echo '<span>' .
                select_input(
                    " name='indexes[$nd][columns][$t]' title='" .
                        lang(37) .
                        "'",
                    $o ? array_combine($o, $o) : $o,
                    $d,
                    'partial(' .
                        ($t == count($w['columns'])
                            ? 'indexesAddColumn'
                            : 'indexesChangeColumn') .
                        ", '" .
                        js_escape($y == 'sql' ? '' : $_GET['indexes'] . '_') .
                        "')"
                ),
                    $y == 'sql' || $y == 'mssql'
                        ? "<input type='number' name='indexes[$nd][lengths][$t]' class='size' value='" .
                            h($w['lengths'][$z]) .
                            "' title='" .
                            lang(100) .
                            "'>"
            ...
            echo "<td><input name='indexes[$nd][name]' value='" .
            h($w['name']) .
            "' autocapitalize='off' aria-labelledby='label-name'>\n",
                "<td><input type='image' class='icon' name='drop_col[$nd]' src='" .
                    h(
                        preg_replace('~\\?.*~', '', ME) .
                            '?file=cross.gif&version=4.8.1'
                    ) .
...
';
    ...
    if ($_POST && !$m && !isset($_POST['add_x'])) {
        ...
        if ($_POST['drop']) {
            ...
            query_redirect(
                'ALTER DATABASE ' .
                    idf_escape($E) .
                    (preg_match('~^[a-z0-9_]+$~i', $L['collation'])
                        ? " COLLATE $L[collation]"
                        : ''),
                substr(ME, 0, -1),
                lang(180)
    ...
    page_header(DB != '' ? lang(56) : lang(109), $m, [], h(DB));
    $Xa = collations();
    $E = DB;
    if ($_POST) {
        $E = $L['name'];
...
';
    ...
    $Ag = array_keys(
        fields(in_array($L['table'], $uf) ? $L['table'] : reset($uf))
    );
    $se = "this.form['change-js'].value = '1'; this.form.submit();";
    echo '<p>' .
        lang(188) .
        ': ' .
        html_select('table', $uf, $L['table'], $se) .
        "\n";
    if ($y == 'pgsql') {
        echo lang(189) .
            ': ' .
...
';
    ...
    if ($_POST && !$m) {
        ...
        if (
            ...
            query_redirect(
                ($y == 'mssql' ? 'ALTER' : 'CREATE OR REPLACE') .
                    ' VIEW ' .
                    table($E) .
                    $ua,
                $B,
    ...
    page_header($b != '' ? lang(32) : lang(195), $m, ['table' => $b], h($b));
    echo '
<form action="" method="post">
<p>',
        lang(176),
...
',
        support('materializedview')
            ? ' ' . checkbox('materialized', 1, $L['materialized'], lang(122))
            : '',
        '<p>';
            ...
            $Jf =
                ...
                ($L['INTERVAL_VALUE']
                    ? 'EVERY ' .
                        q($L['INTERVAL_VALUE']) .
                        " $L[INTERVAL_FIELD]" .
                        ($L['STARTS'] ? ' STARTS ' . q($L['STARTS']) : '') .
                        ($L['ENDS'] ? ' ENDS ' . q($L['ENDS']) : '')
                    : 'AT ' . q($L['STARTS'])) .
                ' ON COMPLETION' .
                ($L['ON_COMPLETION'] ? '' : ' NOT') .
                ' PRESERVE';
                ...
                queries(
                    ($aa != ''
                        ? 'ALTER EVENT ' .
                            ...
                            $Jf .
                            ($aa != $L['EVENT_NAME']
                                ? "\nRENAME TO " . idf_escape($L['EVENT_NAME'])
                                : '')
                        : 'CREATE EVENT ' .
                            idf_escape($L['EVENT_NAME']) .
                            $Jf) .
                        "\n" .
                        $jg[$L['STATUS']] .
                        ' COMMENT ' .
...
<tr><th>',
        lang(176),
        '<td><input name="EVENT_NAME" value="',
        h($L['EVENT_NAME']),
        '" data-maxlength="64" autocapitalize="off">
<tr><th title="datetime">',
        lang(201),
        '<td><input name="STARTS" value="',
        h("$L[EXECUTE_AT]$L[STARTS]"),
        '">
<tr><th title="datetime">',
        lang(202),
        '<td><input name="ENDS" value="',
        h($L['ENDS']),
        '">
<tr><th>',
        lang(203),
        '<td><input type="number" name="INTERVAL_VALUE" value="',
        h($L['INTERVAL_VALUE']),
        '" class="size"> ',
        html_select('INTERVAL_FIELD', $gd, $L['INTERVAL_FIELD']),
        '<tr><th>',
        lang(112),
        '<td>',
        html_select('STATUS', $jg, $L['STATUS']),
        '<tr><th>',
        lang(39),
        '<td><input name="EVENT_COMMENT" value="',
        h($L['EVENT_COMMENT']),
        '" data-maxlength="64">
<tr><th><td>',
        checkbox(
...
';
    ...
    if ($_POST && !process_fields($L['fields']) && !$m) {
        ...
        drop_create(
            "DROP $Ff " . routine_id($ca, $Ce),
            create_routine($Ff, $L),
            "DROP $Ff " . routine_id($L['name'], $L),
            create_routine($Ff, ['name' => $Cg] + $L),
            "DROP $Ff " . routine_id($Cg, $L),
            substr(ME, 0, -1),
            lang(205),
            lang(206),
            lang(207),
    ...
    page_header(
        $ca != ''
            ? (isset($_GET['function']) ? lang(208) : lang(209)) . ': ' . h($ca)
            : (isset($_GET['function'])
                ? lang(210)
                : lang(211)),
        $m
...
',
        $Gf
            ? lang(19) .
                ': ' .
                html_select('language', $Gf, $L['language']) .
                "\n"
            : '',
        '<input type="submit" value="',
        lang(14),
...
';
    ...
    $E = $_GET['name'];
    $Wg = trigger_options();
    $L = (array) trigger($E, $b) + ['Trigger' => $b . '_bi'];
    if ($_POST) {
        if (
            !$m &&
            in_array($_POST['Timing'], $Wg['Timing']) &&
            in_array($_POST['Event'], $Wg['Event']) &&
            in_array($_POST['Type'], $Wg['Type'])
        ) {
            $qe = ' ON ' . table($b);
            ...
            if ($_POST['drop']) {
                ...
                queries_redirect(
                    $B,
                    $E != '' ? lang(214) : lang(215),
                    queries(create_trigger($qe, $_POST))
                );
                if ($E != '') {
                    queries(
                        create_trigger($qe, $L + ['Type' => reset($Wg['Type'])])
                    );
                }
            }
        }
    ...
    page_header($E != '' ? lang(216) . ': ' . h($E) : lang(217), $m, [
        'table' => $b,
    ]);
    echo '
<form action="" method="post" id="form">
...
<tr><th>',
        ...
        in_array('UPDATE OF', $Wg['Event'])
            ? " <input name='Of' value='" . h($L['Of']) . "' class='hidden'>"
            : '',
        '<tr><th>',
        lang(38),
        '<td>',
        html_select('Type', $Wg['Type'], $L['Type']),
        '</table>
<p>',
        lang(176),
        ': <input name="Trigger" value="',
        h($L['Trigger']),
        '" data-maxlength="64" autocapitalize="off">
',
...
';
    ...
    foreach (get_rows('SHOW PRIVILEGES') as $L) {
        foreach (
            explode(',', $L['Privilege'] == 'Grant option' ? '' : $L['Context'])
            as $ib
        ) {
    ...
    if (
        isset($_GET['host']) &&
        ($J = $g->query('SHOW GRANTS FOR ' . q($ea) . '@' . q($_GET['host'])))
    ) {
        while ($L = $J->fetch_row()) {
            if (
                preg_match('~GRANT (.*) ON (.*) TO ~', $L[0], $C) &&
                preg_match_all(
                    '~ *([^(,]*[^ ,(])( *\([^)]+\))?~',
                    $C[1],
                    $Jd,
        ...
        if ($_POST['drop']) {
            ...
            if (!$m) {
                ...
                foreach ($be as $je => $Hc) {
                    ...
                    if (isset($_GET['grant'])) {
                        $Cf = array_diff(
                            array_keys(array_filter($be[$je], 'strlen')),
                            $Hc
                        );
                    } elseif ($pe == $de) {
                    ...
                    if (
                        preg_match('~^(.+)\s*(\(.*\))?$~U', $je, $C) &&
                        (!grant('REVOKE', $Cf, $C[2], " ON $C[1] FROM $de") ||
                            !grant('GRANT', $Hc, $C[2], " ON $C[1] TO $de"))
                    ) {
                        $m = true;
                        break;
                    }
            ...
            queries_redirect(
                ME . 'privileges=',
                isset($_GET['host']) ? lang(221) : lang(222),
                !$m
            );
            if ($mb) {
    ...
    if ($_POST) {
        ...
        $Ic[
            (DB == '' || $Ic ? '' : idf_escape(addcslashes(DB, '%_\\'))) . '.*'
        ] = [];
    }
    echo '<form action="" method="post">
<table cellspacing="0" class="layout">
<tr><th>',
        lang(23),
        '<td><input name="host" data-maxlength="60" value="',
        h($L['host']),
        '" autocapitalize="off">
<tr><th>',
        lang(24),
        '<td><input name="user" data-maxlength="80" value="',
        h($L['user']),
        '" autocapitalize="off">
<tr><th>',
        lang(25),
        '<td><input name="pass" id="pass" value="',
        h($L['pass']),
...
';
    ...
    foreach ($Ic as $je => $Hc) {
        echo '<th>' .
            ($je != '*.*'
                ? "<input name='objects[$t]' value='" .
                    h($je) .
                    "' size='10' autocapitalize='off'>"
                : "<input type='hidden' name='objects[$t]' value='*.*' size='10'>*.*");
        $t++;
    }
    echo "</thead>\n";
    foreach (
        ...
        foreach ((array) $jf[$ib] as $if => $bb) {
            ...
            foreach ($Ic as $je => $Hc) {
                ...
                if (
                    ...
                    echo "<td><select name=$E><option><option value='1'" .
                        ($Y ? ' selected' : '') .
                        '>' .
                        lang(225) .
                        "<option value='0'" .
...
',
    ...
    foreach (process_list() as $t => $L) {
        ...
        echo '<tr' .
            ...
            '>' .
            (support('kill')
                ? '<td>' . checkbox('kill[]', $L[$y == 'sql' ? 'Id' : 'pid'], 0)
                : '');
        foreach ($L as $z => $X) {
            echo '<td>' .
                (($y == 'sql' &&
                    $z == 'Info' &&
                    preg_match('~Query|Killed~', $L['Command']) &&
                    $X != '') ||
                ($y == 'pgsql' && $z == 'current_query' && $X != '<IDLE>') ||
                ($y == 'oracle' && $z == 'sql_text' && $X != '')
                    ? "<code class='jush-$y'>" .
...
';
    if (support('kill')) {
        echo $t + 1 . '/' . lang(229, max_connections()),
            "<p><input type='submit' value='" . lang(230) . "'>\n";
    }
    echo '<input type="hidden" name="token" value="',
        $T,
        '">
        ...
        echo DB != ''
            ? '<input type="hidden" name="db" value="' .
                ...
                (isset($_GET['ns'])
                    ...
                    : '')
            : '';
        echo '<input type="hidden" name="select" value="' . h($b) . '">',
            "</div>\n";
        ...
        if (!$J) {
                    ...
                    foreach ($dh as $z => $X) {
                        if (
                            ...
                            $z =
                                'MD5(' .
                                ($y != 'sql' ||
                                preg_match('~^utf8~', $o[$z]['collation'])
                                    ? $z
                                    : "CONVERT($z USING " . charset($g) . ')') .
                                ')';
                            $X = md5($X);
                        }
                        $eh .=
                    ...
                    echo '<tr' .
                        ...
                        (!$s && $N
                            ...
                            : '<td>' .
                                ...
                                ($kd || information_schema(DB)
                                    ? ''
                                    : " <a href='" .
                                        h(ME . 'edit=' . urlencode($b) . $eh) .
                                        "' class='edit'>" .
                                        lang(238) .
                                        '</a>'));
                    foreach ($L as $z => $X) {
                        if (isset($ae[$z])) {
                            ...
                            if (($_GET['modify'] && $Sb) || $Y !== null) {
                                ...
                                echo '>' .
                                    ($Eg
                                        ? "<textarea name='$u' cols='30' rows='" .
                                            (substr_count($L[$z], "\n") + 1) .
                                            "'>$Nc</textarea>"
                                        : "<input name='$u' value='$Nc' size='$Cd[$z]'>");
                            } else {
                                $Gd = strpos($X, '<i>…</i>');
                                echo " data-text='" .
                                    ($Gd ? 2 : ($Eg ? 1 : 0)) .
            ...
            if (!is_ajax()) {
                if ($M || $F) {
                    ...
                    if ($_GET['page'] != 'last') {
                        if ($_ == '' || (count($M) < $_ && ($M || !$F))) {
                            ...
                            if ($Ec < max(1e4, 2 * ($F + 1) * $_)) {
                                $Ec = reset(
                                    slow_query(count_rows($b, $Z, $kd, $s))
                                );
                            } else {
                                $ic = false;
                            }
                    ...
                    if ($Ke) {
                        echo ($Ec === false ? count($M) + 1 : $Ec - $F * $_) >
                    $_
                        ? '<p><a href="' .
                            h(remove_from_uri('page') . '&page=' . ($F + 1)) .
                            '" class="loadmore">' .
                            lang(240) .
                            '</a>' .
                            script(
...
<input type="submit" name="edit" value="',
                            lang(10),
                            '">
<input type="submit" name="clone" value="',
                            lang(228),
                            '">
<input type="submit" name="delete" value="',
                            lang(18),
...
';
                    ...
                    if ($Cc) {
                        print_fieldset(
                            'export',
                            lang(62) . " <span id='selected2'></span>"
                        );
                        $Ie = $c->dumpOutput();
                        echo $Ie
                        ? html_select('output', $Ie, $ma['output']) . ' '
                        : '',
                            html_select('format', $Cc, $ma['format']),
                            " <input type='submit' name='export' value='" .
                                lang(62) .
                                "'>\n",
                            "</div></fieldset>\n";
    ...
    header('Content-Type: text/javascript; charset=utf-8');
    if ($_GET['script'] == 'db') {
        $rg = ['Data_length' => 0, 'Index_length' => 0, 'Data_free' => 0];
                ...
                foreach (
                    ...
                    if ($R[$z] != '') {
                        ...
                        json_row(
                            ...
                            $X &&
                            $R['Engine'] ==
                                ($dg == 'pgsql' ? 'table' : 'InnoDB')
                                ? "~ $X"
                                : $X
                        );
                        if (isset($rg[$z])) {
                            $rg[$z] +=
                                $R['Engine'] != 'InnoDB' || $z != 'Data_free'
                                    ? $R[$z]
                                    : 0;
                        }
    ...
    if ($c->homepage()) {
        if ($_GET['ns'] !== '') {
            ...
            if (!$yg) {
                    ...
                    if ($vh) {
                        ...
                        foreach (
                            [
                                'Engine' => [],
                                'Collation' => [],
                                'Data_length' => ['create', lang(33)],
                                'Index_length' => ['indexes', lang(126)],
                                'Data_free' => ['edit', lang(34)],
                                'Auto_increment' => [
                ...
                echo '<tr><td><th>' . lang(229, count($yg)),
                    '<td>' .
                        h(
                            $y == 'sql'
                                ? $g->result('SELECT @@default_storage_engine')
                                : ''
                        ),
            ...
            if (support('routine')) {
                ...
                if ($Hf) {
                    ...
                    foreach ($Hf as $L) {
                        $E =
                            $L['SPECIFIC_NAME'] == $L['ROUTINE_NAME']
                                ? ''
                                : '&name=' . urlencode($L['ROUTINE_NAME']);
                        echo '<tr' . odd() . '>',
                            '<th><a href="' .
                                h(
                                    ME .
                                        ($L['ROUTINE_TYPE'] != 'PROCEDURE'
                                            ? 'callf='
                                            : 'call=') .
                                        urlencode($L['SPECIFIC_NAME']) .
                                        $E
                                ) .
                                '">' .
                                h($L['ROUTINE_NAME']) .
