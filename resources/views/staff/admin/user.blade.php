    ...
    <div class="mt-10">
        ...
        <form action="{{ route('put.admin.edituser', ['user' => $user->id]) }}" method="post">
            ...
            <div class="inblock">
                ...
                <input type="radio" id="finalizearly" name="finalizearly" value="1" @if($user->finalizeEarly()) checked
                @endif>yes
                <input type="radio" id="finalizearly" name="finalizearly" value="0" @if(!$user->finalizeEarly()) checked
                @endif>no
            </div>
            <br>
            ...
            <input type="checkbox" id="seller" name="role[]" value="seller" @if($user->isSeller()) checked @endif><label
                for="seller">seller</label>
            <input type="checkbox" id="moderator" name="role[]" value="moderator" @if($user->isModerator()) checked
            @endif><label for="moderator">moderator</label>
            <input type="checkbox" id="admin" name="role[]" value="admin" @if($user->isAdmin()) checked @endif><label
