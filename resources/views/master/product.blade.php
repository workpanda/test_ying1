...
<div class="content-master">
    ...
    <div class="inblock">
        ...
        <div class="mt-10">
            ...
            <div class="inblock">
                <a href="#image{{ $index }}">
                    <img src="{{ $image->image }}" alt="{{ $product->name }}" width="72px" height="72px">
                </a>
            </div>
            @endforeach
        </div>
    ...
    <div class="product-alldetails inblock" style="width: 538px">
        ...
        <div class="mt-20">
            <span class="price-big"><small>from</small>
                @include('includes.components.displayprice', ['price' => $product->from()])
            </span>&ensp;&ensp;
            <a href="{{ route('report', ['product' => $product->id]) }}" target="_blank">report this product</a>
        </div>
        <div class="container mt-20">
            seller: <a
                href="{{ route('seller', ['seller' => $product->seller->username]) }}">{{ $product->seller->username }}({{ $product->seller->totalFeedbacks() }})</a><br>
            ships from: {{ $product->shipsFrom() }}<br>
            ships to: {{ $product->shipsTo() }}<br>
            finalize early (present):
            <strong>{{ $product->seller->finalizeEarly() == true ? 'yes' : 'no' }}</strong><br>
            category: @foreach($product->category->parents() as $pc) <a
                href="{{ route('category', ['slug' => $pc->slug]) }}">{{ $pc->name }}</a> &rarr; @endforeach <a
            ...
            <form action="{{ route('post.addtocart', ['product' => $product->id]) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <select id="offer" name="offer" class="dropdown-wrapper">
                        ...
                        <option value="{{ $offer->id }}">{{ $offer->quantity }} {{ $offer->mesure }} per
                            @include('includes.components.displayprice', ['price' => $offer->price])
                        </option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select id="delivery_method" name="delivery_method" class="dropdown-wrapper">
                        ...
                        <option value="{{ $delivery->id }}">{{ $delivery->name }} - {{ $delivery->days }} day(s) -
                            @include('includes.components.displayprice', ['price' => $delivery->price])
                        </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="inblock">
                    <button><a href="{{ route('images', ['section' => 'edit', 'product' => $product->id]) }}"
                            class="button">edit this product</a></button>
                </div>
                @endcan
            </form>
        </div>
