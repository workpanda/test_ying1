...
<div class="content-browsing">
    ...
    <div class="h2 mb-15">Order - <a
            href="{{ route('product', ['product' => $order->product_id]) }}">{{ $order->product->name }}</a></div>
    <div class="title">
    ...
    <div class="inblock" style="vertical-align: top">
        <table class="zebra">
            ...
            <tr>
                <td>total price</td>
                <td>{{ \App\Tools\Converter::currencyConverter($order->total, auth()->user()->currency) }}
    ...
    <div class="col mt-40">
        ...
        <div class="inblock" style="vertical-align: top; width: 606px">
            <table class="zebra" style="width: 100%">
                ...
                <tbody>
                    <tr>
                        <td><input type="text" value="{{ $order->escrow_monero_wallet }}"
                                style="font-family: Courier; font-size: 90%; background-color: #fff; color: #000; font-weight: bold; width: 261px"
                                disabled></td>
                        <td class="text-center">XMR {{ $totalSent }}</td>
                        <td class="text-center">XMR {{ $toPay }}</td>
                        <td>
                            <div class="flashdata flashdata-warning">{{ $order->paidOrder() ? 'yes' : 'no' }}</div>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <div class="footnote container mt-10">
                ...
                @if($order->waiting())
                @if($order->isBuyer())
                <span>Pay for the purchase and wait for the seller to accept your order. If the seller does not accept,
                    your money will be refunded.
                    <strong>Orders with the status "waiting" are automatically canceled within two days.</strong>
                </span>
                @else
                <span>You can only accept the order after the buyer sends the money to the payment wallet.
                    <strong>Orders with the status "waiting" are automatically canceled within two days.</strong>
                </span>
                @endif
                ...
                <div class="actions mt-20">
                    @if(!$order->isBuyer())
                    <form action="{{ route('post.changeorderstatus', ['order' => $order->id, 'status' => 'shipped']) }}"
                        method="post" class="inblock">
                        @csrf
                ...
                @endif
                @if($order->shipped())
                <span>The order has been marked as shipped! <strong>Purchases with the status "shipped" complete
                        automatically within thirty days.</strong></span>
                <div class="actions mt-20">
                    @if($order->isBuyer())
                    <form
                        action="{{ route('post.changeorderstatus', ['order' => $order->id, 'status' => 'delivered']) }}"
                        method="post" class="inblock">
                        @csrf
                    ...
                    <form action="{{ route('post.finalizearly', ['order' => $order->id]) }}" method="post"
                        class="inblock float-right">
                        @csrf
                ...
                @if($order->delivered())
                @if($order->isBuyer())
                <span>You marked the order as delivered and the funds have been released to the seller. In case of
                    problems with the received product, please start a dispute.</span>
                <div class="actions mt-20">
                    <form
                        action="{{ route('post.changeorderstatus', ['order' => $order->id, 'status' => 'disputed']) }}"
                        method="post" class="inblock">
                ...
                <form action="{{ route('post.feedback', ['feedback' => $feedback->id]) }}" method="post"
                    class="mt-20 text-default">
                    ...
                    <div class="form-group inblock">
                        ...
                        <input type="text" id="rating" name="rating" placeholder="from 1 to 5" @if(!is_null($feedback))
                            value="{{ $feedback->rating }}" @endif>
                    </div>
                    <div class="form-group inblock">
                        <div class="label">
                        ...
                        <select id="type" name="type" class="dropdown-wrapper">
                            ...
                            <option value="{{ $type }}" @if(!is_null($feedback) and $feedback->type == $type) selected
                                @endif>{{ $type }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <textarea id="feedback" name="feedback" cols="50"
                            rows="20">@if(!is_null($feedback)) {{ $feedback->message }} @endif</textarea>
                    </div>
                    <button type="submit">submit feedback</button>
                </form>
                @else
                <span>Order marked as delivered! Funds have been released. Money will drop after two
                    confirmations.</span>
                <div class="actions mt-20">
                    <form
                        action="{{ route('post.changeorderstatus', ['order' => $order->id, 'status' => 'disputed']) }}"
                ...
                <span class="flashdata flashdata-success mt-10 mb-10">Winner of the dispute:
                    {{ $order->dispute->winner->username }}</span>
                @else
                <span class="flashdata flashdata-error mt-10 mb-10">Winner of the dispute: none</span>
                @endif
                @staff
                <form action="{{ route('post.resolvedispute', ['dispute' => $order->dispute->id]) }}" method="post"
                    class="mb-15">
                    @csrf
                    <label id="winner" class="text-default">winner:</label>
                    <select id="winner" name="winner" class="dropdown-wrapper">
                        <option value="">none</option>
                        <option value="{{ $order->buyer->id}}" @if($order->dispute->winner_id == $order->buyer->id)
                            selected @endif>{{ $order->buyer->username }} (buyer)</option>
                        <option value="{{ $order->seller->id}}" @if($order->dispute->winner_id == $order->seller->id)
                            selected @endif>{{ $order->seller->username }} (seller)</option>
                    </select>
                    <button type="submit">mark</button>
                </form>
                @endif
                <form action="{{ route('post.createdisputemessage', ['dispute' => $order->dispute->id]) }}"
                    method="post" class="text-default">
                    @csrf
