<div class="product">
    ...
    <div class="product-price">
        <div class="price-big"><small>from</small> @include('includes.components.displayprice', ['price' =>
            $product->from()])</div>
        <form action="{{ route('post.favorites', ['product' => $product->id]) }}" method="post">
            @csrf
            <button type="submit" class="btn-link">{{ auth()->user()->isFavorite($product) ? 'remove' : 'add' }} to
                favorite</button>
        </form>
    </div>
    <div class="product-infos">
        ...
        <div class="product-description mt-15">
            <span>seller: <a
                    href="{{ route('seller', ['seller' => $product->seller->username]) }}">{{ $product->seller->username }}({{ $product->seller->totalFeedbacks() }})</a></span><br>
