@extends('master.main')
@section('content')
<title>Orders - Squid Market</title>

<div class="content-master">
    <div class="h2 mb-15">Orders</div>
    <a href="{{ route('orders', ['status' => 'all']) }}" class="container">all <span
            class="nav-count">{{ $user->orders->count() }}</span></a>
    <a href="{{ route('orders', ['status' => 'waiting']) }}" class="container">waiting <span
            class="nav-count">{{ $user->totalOrders('waiting') }}</span></a>
    <a href="{{ route('orders', ['status' => 'accepted']) }}" class="container">accepted <span
            class="nav-count">{{ $user->totalOrders('accepted') }}</span></a>
    <a href="{{ route('orders', ['status' => 'shipped']) }}" class="container">shipped <span
            class="nav-count">{{ $user->totalOrders('shipped') }}</span></a>
    <a href="{{ route('orders', ['status' => 'delivered']) }}" class="container">delivered <span
            class="nav-count">{{ $user->totalOrders('delivered') }}</span></a>
    <a href="{{ route('orders', ['status' => 'canceled']) }}" class="container">canceled <span
            class="nav-count">{{ $user->totalOrders('canceled') }}</span></a>
    <a href="{{ route('orders', ['status' => 'disputed']) }}" class="container">disputed <span
