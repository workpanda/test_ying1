...
class jsonRPCClient
{
    ...
    public function __construct($pUrl, $pUser, $pPass, $check_SSL)
    {
        $this->validate(
            ...
            'The curl extension must be loaded to use this class!'
        );
        $this->validate(
            false === extension_loaded('json'),
            'The json extension must be loaded to use this class!'
        );
    ...
    public function setCurlOptions($pOptionsArray)
    {
        if (is_array($pOptionsArray)) {
            ...
            throw new InvalidArgumentException(
                'Oops... An error occurred, please try later!'
            );
        }
        return $this;
    }
    ...
    public function _run($pMethod, $pParams, $path)
    {
        ...
        $this->validate(
            !is_null($jsonErrorMsg) && $jsonErrorMsg !== 'No error',
            $jsonErrorMsg . ': ' . $responseMessage
        );
        if (isset($responseDecoded['error'])) {
            $errorMessage =
                'Request have return error: ' .
                $responseDecoded['error']['message'] .
                '; ' .
                "\n" .
                'Request: ' .
                $request .
