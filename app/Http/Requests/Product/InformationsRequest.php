...
class InformationsRequest extends FormRequest
{
    ...
    public function add()
    {
        ...
        $totalImages = session()->has('images')
            ? count(session()->get('images'))
            : 0;

        if ($totalImages == 0) {
            throw new \Exception(
                'You need to add at least one image to the product!'
            );
        }

        #Check if there are offers in the session and how many there are
        $totalOffers = session()->has('offers')
            ? count(session()->get('offers'))
            : 0;

        if ($totalOffers == 0) {
            throw new \Exception(
                'You need to add at least one offer to the product!'
            );
        }

        #Check if there are delivery methods in the session and how many there are
        $totalDeliveryMethods = session()->has('deliveries')
            ? count(session()->get('deliveries'))
            : 0;

        if ($totalDeliveryMethods == 0) {
            throw new \Exception(
                'You need to add at least one delivery method for the product!'
            );
        }
