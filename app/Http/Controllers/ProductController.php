...
use App\Models\{Product, Image, Offer, Delivery, Category, Favorite, Livefeeds};
...
class ProductController extends Controller
{
    ...
    public function createPhysical($section, Product $product = null)
    {
        $this->product();

        if ($section === 'add' && is_null($product)) {
            return view('includes.forms.product.create-physical-listing', [
                'product' => $product,
    ...
    public function createDigital($section, Product $product = null)
    {
        $this->product();

        if ($section === 'add' && is_null($product)) {
            return view('includes.forms.product.create-digital-listing', [
                'product' => $product,
                'categories' => Category::get(),
                'section' => $section,
            ]);
    ...
    public function postPhysicalProduct(
        $section,
    ...
    public function postDigitalProduct(
        $section,
        Product $product = null,
        DigitalProductRequest $request
    ) {
        $this->product();
     ...
     *
     * @return App\Http\Requests\Product\ProductRequest
     */
        ...
        if ($section === 'add' && is_null($product)) {
            ...
            try {
                ...
                if (count($product->offers) == 1) {
                    throw new \Exception(
                        'The product must have at least one offer!'
                    );
                }
    ...
    public function viewDeliveries($section, Product $product = null)
    {
        ...
        if ($section === 'add' && is_null($product)) {
            return view('includes.forms.product.deliveries', [
                'deliveries' => session()->has('deliveries')
                    ? session()->get('deliveries')
                    : [],
                'section' => $section,
            ]);
        } elseif (
     ...
     * @return Illuminate\Routing\Redirector
     */
    public function postDeleteDelivery(
        $section,
        $delivery,
        Product $product = null
    ) {
        ...
        if ($section === 'add' && is_null($product)) {
            ...
            $deliveriesMethods = session()->has('deliveries')
                ? session()->get('deliveries')
                : [];
            ...
            try {
                ...
                if (count($product->deliveries) == 1) {
                    throw new \Exception(
                        'The product must have at least one delivery method!'
                    );
                }
     ...
     */
    public function postInformations(
        $section,
        Product $product = null,
        InformationsRequest $request
    ) {
        $this->product();
    ...
    public function viewConfirms(Product $product)
    {
        return view('includes.flash.are-you-sure', [
            'product' => $product,
        ]);
    }
}