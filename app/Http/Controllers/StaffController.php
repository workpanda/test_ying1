...
class StaffController extends Controller
{
    ...
    public function putFeatured(Product $product)
    {
        if (!$product->featured) {
            ...
            session()->flash(
                'success',
                'Product excluded from the highlights!'
            );
    ...
    public function viewSupport(Request $request)
    {
        ...
        if ($status == 'closed') {
            $helpRequests = HelpRequest::where('closed', true)
                ->orderBy('closed')
                ->paginate(1);
        } elseif ($status == 'open') {
            $helpRequests = HelpRequest::where('closed', false)
                ->orderBy('closed')
                ->paginate(25);
        } else {
    ...
    public function postOpenHelpRequest(HelpRequest $helpRequest)
    {
        $helpRequest->status = 'accepted';
        $helpRequest->admin_id = auth()->user()->id;
        $helpRequest->save();
    ...
    public function postResolveDispute(Request $request, Dispute $dispute)
    {
        try {
            if (
                $request->winner != $dispute->buyer->id and
                $request->winner != $dispute->seller->id
            ) {
