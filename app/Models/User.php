...
use App\Models\{Conversation, Product};
...
class User extends Authenticatable
{
    ...
    {
        return Conversation::where('issuer_id', $this->id)->orWhere(
            'receiver_id',
            $this->id
        );
    }
    ...
    {
        return Product::get()->where('seller_id', $this->id);
    }
    /**
    ...
    {
        return $this->hasMany(HelpRequest::class, 'user_id', 'id')->orderBy(
            'created_at',
            'DESC'
        );
    }
    ...
    {
        return $this->hasMany(Transition::class, 'user_id', 'id')->orderBy(
            'created_at',
            'DESC'
        );
    }
    ...
    {
        return $this->hasMany(Product::class, 'seller_id', 'id')->where(
            'deleted',
            false
        );
    }
    ...
    {
        return $this->hasMany(Feedback::class, 'seller_id', 'id')->where(
            'message',
            '!=',
            ''
        );
    ...
    /**
     * Get count of New Users in 24 hours
     * @param  $mounths
     *
     * @return int
     */
    public static function countofNewUsers()
    {
    ...
    public function ratePositiveFeedbacks()
    {
        ...
        $rate =
            $totalFeedbacks > 0
                ? ($totalPositiveFeedbacks * 10) / $totalFeedbacks
                : 0;
    ...
    {
        $totalDisputes = Dispute::where('seller_id', $this->id)->count();
    ...
    {
        $balanceWallet = \Monerod::getTotalReceived(
            $this->become_monero_wallet
        );
